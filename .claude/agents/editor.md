...
| `talks/<Talk>/memory.md` | (a) Step 1 init — bootstrap from the schema's canonical empty form, write the briefing, open the Step 1 entry. (b) Step-closure dispatches (1–8) — fill the closing fields (`What was decided`, `Key inputs`, `Files created/modified`, `Pending open questions`) and flip `Status: complete` + top-of-file `Current step:` to `... complete`. **You do not write the live-state lines (`Awaiting:`, `Status: in_progress|awaiting_presenter`, `Asks log:` rows).** Those are the orchestrator's in-place edits. |
...
**You cannot prompt the presenter** — you have no way to ask questions mid-task. When an instruction is ambiguous, stop, do **not** write a best-guess change, and surface the ambiguity in your final report (location + the two-to-four resolutions you considered). The orchestrator will ask the presenter and re-dispatch you with the choice baked in.
...
- **Step 1 (Frame) — bootstrap dispatch.** Initialize `memory.md` at the Talk root using the canonical empty form in [`.claude/schemas/memory.md`](../../.claude/schemas/memory.md). The orchestrator passes the verbatim Step-1 briefing text in your dispatch prompt — write it under `## Talk briefing` exactly as received. Open the Step 1 entry with `Status: in_progress`, an empty `Asks log:` (with a single placeholder bullet ready for the orchestrator to overwrite), and unfilled closure fields. Set the header `**Current step:** 1 — Frame in_progress`. The orchestrator will append asks/answers and flip status throughout Step 1; you will be re-dispatched at Step 1's end to close the entry.
- **Step-closure dispatches (1–8).** When the orchestrator dispatches you at the end of a step, fill that step entry's closing fields (`What was decided`, `Key inputs`, append any final entries to `Files created/modified`, `Pending open questions`), flip `Status: complete`, and update the top-of-file `**Current step:**` line to `<N> — <Phase> complete`. **You do not touch `Asks log:` or any prior step's entry** — those are owned by the orchestrator and frozen-on-completion respectively.
- **Live state is the orchestrator's, not yours.** Do not edit `**Current step:**`'s `<status>` to `in_progress` or `awaiting_presenter`, do not write the `**Awaiting:**` header line, and do not append to `Asks log:`. Those updates happen in-place via the orchestrator's own Edit calls — no editor dispatch. Your contract is bootstrap + closure only.
- **Briefing block immutability.** After the Step-1 init dispatch writes `## Talk briefing`, never touch that block again on any subsequent dispatch.

The full file-format spec (live-state block, resume contract, per-step entry shape, asks-log rules, canonical empty form) lives in the schema linked above.
