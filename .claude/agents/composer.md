...
- **You cannot prompt the presenter** — you have no way to ask questions mid-task. If a critique requires presenter input to resolve (e.g. "this section could be reordered two ways — which does the presenter prefer?"), tag the item additionally with `[needs-presenter-input]`. The orchestrator surfaces these to the presenter rather than auto-feeding them back to the editor.
