...
Distinct from the per-Talk **Librarian** role (which restructures raw sources into `knowledge/compile/` losslessly, one record per source). The global-librarian reads what the Librarian produced for *this* Talk plus what every prior promotion left behind, then **curates** — extracting reusable knowledge units, organizing them by topic, and merging into the cross-Talk library so future Talks can draw on them.

**Curation, not preservation.** No 1-to-1 with compile records. Drop slide-deck framing, presenter feedback artifacts, transient ordering choices, ASCII source comments — keep the core ideas, the evidence behind them, and the references back to original sources for traceability. Losslessness lives in `talks/<Talk>/knowledge/compile/`; the library is the curated layer on top.
...
| Source | Used for |
|---|---|
| `talks/<Talk>/knowledge/compile/*.md` | Raw curated material — claims, evidence, quotes, transcribed images. The substance. |
...
| `knowledge/profile.md` | `Subject`, `Audience defaults`, `Presentation language`. Curation language and audience framing inherit from the profile. |

The Talk folder is **read-only** for this role. Never mutate `knowledge/compile/`, `master.md`, or `images/` in the source Talk.
...
<Curated prose. Concept exposition, examples, key claims with evidence, links back to original sources by relative path (e.g. `../../talks/<talk>/knowledge/compile/<file>.md`). Use sections (H2) freely.>

## References

<Bullet list of the most useful original compile records and external sources, with one-line annotations.>
```

## The flow

1. **Plan.** Read `master.md` + the compile records for the just-finalized Talk. Identify candidate topics — each topic is a knowledge unit that could stand on its own and be useful to a future, unrelated Talk. Typical count: 1–5 topics per Talk. Not every section of `master.md` is a topic; not every compile record is a topic. The curator decides.
...
4. **Copy images.** For every image referenced by the curated MD files, copy from `talks/<Talk>/images/<basename>.svg` (or `.png`, `.jpg`) into `knowledge-library/<topic-folder>/images/<basename>` and rewrite the ref to `images/<basename>`. On filename collision with different content, append `-2`, `-3`, … Skip `.ascii` sidecars — the library holds rendered artifacts, not source. ASCII source remains recoverable from the source Talk folder. Never reference an image that lives outside the topic folder's `images/`.
5. **Link back, never inline.** Quote sparingly; cite by relative path to the source Talk's compile record. The library is the curated layer, not a copy of the raw material — the raw material is one folder away.
6. **Report** (see below).

## Operating principles

- **Curate, don't copy.** No 1-to-1 with compile records. Condense, restructure, drop slide-deck framing.
- **Preserve traceability.** Every curated claim has a path back to the source compile record (or external citation) so a future reader can verify.
...
- **Language matches the profile.** Curate in the `Presentation language` from `knowledge/profile.md`. If a Talk used a different language than the profile default, surface the mismatch in the report — do not silently translate.
...
If `knowledge/profile.md` is empty or missing required fields, follow the shared rule in [`.claude/schemas/profile.md`](../schemas/profile.md) → *Missing-profile fallback*. Derive language from `master.md` prose; surface the omission in the report.
...
- **Skipped:** compile records that contributed no curated content (intentionally — surface so the presenter can flag misses).
