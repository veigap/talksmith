...
| `ascii_file_rel` / `output_path_rel` / `talk_rel` | optional | Talk-portable path twins emitted by `polish-ascii prepare-render-args` (`ascii_file_rel`/`output_path_rel` are `images/<name>`; `talk_rel` is the Talk dir relative to `repo_root`). Present in a normal Step-6 args file; absent for a hand-built Mode-A call. |

**Path resolution — prefer the portable twins.** The absolute `ascii_file` / `output_path` are captured in the *scanning* session's filesystem view; when this render runs in a **different mount** (a worker VM vs the host), those absolute paths point nowhere. So when `talk_rel` and the `*_rel` twins are present, **resolve paths against the `repo_root` you were dispatched with** — read from `<repo_root>/<talk_rel>/<ascii_file_rel>` and write to `<repo_root>/<talk_rel>/<output_path_rel>` — which is correct in *your* mount regardless of where `scan` ran. Fall back to the absolute `ascii_file` / `output_path` only when the twins are absent (a hand-built Mode-A call) or the resolved-relative input doesn't exist. This makes the render mount-independent; the absolute paths remain a same-session fast path, never the sole source of truth.
...
5. **Render the SVG.** Use a `viewBox` proportional to the diagram's aspect ratio (commonly `0 0 680 H` for landscape diagrams). Include `<title>` and `<desc>` as the first two children of the root `<svg>` for accessibility. Choose colors and shapes that satisfy *every* standing rule + any non-conflicting per-render directive. When the diagram has semantic structure (input/output, before/after, baseline/improved), use distinct hues to make the structure visible; default palette is the presenter's choice or a tasteful neutral if no directive is provided.
   ...
   **Aspect-ratio contract — the SVG's intrinsic aspect is load-bearing downstream.** The deck sizes each picture's slot from this SVG's `viewBox` ratio, and the `.pptx` export rasterizes the SVG at the size the browser painted it — so a wrong `viewBox` is a wrong shape everywhere at once, silently and in every format. So:
...
6. **Write the SVG** to `output_path`. Create the parent directory if it doesn't exist (`mkdir -p`). Overwrite if the file already exists — idempotency is the caller's concern, not this skill's.
   ...
   **Then validate it against the aspect-ratio contract** by running [`validate_svg.py`](validate_svg.py) on the file you just wrote:
   ...
   A non-zero exit means the SVG is broken in a way this skill can't mechanically repair (no viewBox, malformed XML, root element isn't `<svg>`). When that happens, **do not return success** — return `failed: svg_validation: <error>` per step 9. A broken SVG must not reach disk: the deck trusts the viewBox to size its slot, and a faulty one is a misshapen diagram in the HTML, the PDF and the `.pptx` at once. Catching it here is cheap; catching it after a full render is not. If repair happened, note the fix count in the step-9 report under `svg_validation:`.
