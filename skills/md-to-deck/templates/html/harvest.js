...
(function () {
  ...
  /* ---- text ---------------------------------------------------------------------------- */
  /* Is everything inside this element part of one text flow, rather than a box of its own?
   * Decided from computed `display`, never from the tag name.
   *
   * A tag whitelist got this catastrophically wrong. The section roadmap's rows are `<a>`
   * elements and `<a>` is nominally inline, so an entire roadmap collapsed into a single text
   * node and exported as one heap of overlapping words. The deck styles many of its `<a>`s and
   * `<span>`s as `flex` or `block`, and only computed style knows which.
   *
   * An inline child that paints is a box in its own right too — a `code` chip has a background
   * and a border, and flattening it into a run would drop both. */
  function inlineOnly(el) {
    for (var i = 0; i < el.children.length; i++) {
      ...
      if (ccs.display.indexOf('inline') !== 0 && ccs.display !== 'contents') return false;
