...
6. **Audit `<p:pic>` aspect ratios.** Run [`audits/aspect_ratios.py`](audits/aspect_ratios.py) against the rendered deck:
   ...
   **Then run [`audits/palette_fonts.py`](audits/palette_fonts.py) — only when `style == strict`.** This enforces the strict template's §2 palette + §3.1 font set (a LAYOUT-CONFORMANCE concern — see [`slide-design.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md)). **Skip for `free-form` and `preview`** — their slides 2+ deliberately have no fixed palette/font, so this audit does not apply (for free-form the fixed cover is still checked by `audits/cover_fidelity.py`; preview produces no deck, so no deck-parsing audit runs there at all). For strict:
