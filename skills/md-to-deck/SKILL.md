...
**Style resolution — `style:` invocation parameter (mandatory, no default).** Every render begins by reading the `style:` parameter the orchestrator passed in at invocation. Allowed values are `pptx-strict`, `pptx-free-form`, and `html-strict` per [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/README.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/README.md). The orchestrator asks the presenter at every Step 8 entry (see [`${CLAUDE_PLUGIN_ROOT}/orchestrator.md`](${CLAUDE_PLUGIN_ROOT}/orchestrator.md) → Step 8 step 1) and must get an explicit answer before dispatching this skill. `final.md` and `draft.md` carry no `style:` field — the same Talk content can be rendered in either style at any time. If the invocation arrives with no `style:` parameter, the skill **fails render-blocking** with the error message at Process step 0 below; it does not guess or default. The resolved style determines two paths the rest of this skill uses:
...
**The base template is mandatory and non-negotiable.** Every render starts from a working copy of `<base_template_path>`, not from a blank deck. For `pptx-strict`: this is a 15-slide foundation (cover + agenda + 12 layout-reference slides + 1 section-divider example) and the renderer substitutes placeholders, deletes the layout-reference zone (slides 3–15), and inserts generated content per the recipes in `<spec_path>`. For `pptx-free-form`: this is a 1-slide cover-only foundation; the renderer substitutes the cover's four §2 placeholders, then builds every other slide fresh per the style's §3 (renderer decides). Decks rendered from scratch — even if they "look similar" — are a render failure in either style.
...
**Reads `final.md`, never `draft.md` — except the `html-strict --draft` live view.** For the native `.pptx` styles the working file (`draft.md`) still carries `Presenter feedback` blocks and raw ASCII fences and is not a valid PPTX input; passing it produces malformed output (feedback bullets leaking into bodies, ASCII rendered as code). The **one exception** is `html-strict` in `--draft` mode (the Step-5.5 live view), which reads `draft.md` *by design* and pre-processes it with `convert.py --draft` — see *`html-strict` on `draft.md`* below. It writes to `output/html/index.html` (the same place as the deliverable) and never mutates `draft.md`.
...
| Visual spec | `<spec_path>` = `${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/<style>/pptx-prompt.md` exists. Section catalog varies by style — strict §1–§20, free-form §1–§4. Per-format render config is the matrix in [`render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md); the design-quality practices its FEEDBACK action walks are in [`slide-design.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md) (pptx-strict + html-strict walk it; pptx-free-form is single-pass). | Stop and ask — the spec is the rendering contract. |
| Icon library *(strict only)* | [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/base-template.pptx`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/base-template.pptx) `ppt/media/icon-*.svg` (15 branded line-art icons in `#DA1B2E`) — see [`pptx-strict/pptx-prompt.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/pptx-prompt.md) §17. Free-form makes icons optional (per `pptx-free-form/pptx-prompt.md` §3.2). | Stop and ask — without the icons the strict no-emoji rule cannot be enforced. |
...
- **Base template** — resolved as `<base_template_path>` from the `style:` invocation parameter (`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/<style>/base-template.pptx`). Override only if the presenter explicitly passes a different path. The legacy [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/template.pptx`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/template.pptx) (53-slide reference deck) is **not** a valid override — it is the source the strict spec was distilled from, not a rendering input.
...
2.5. **Classify each slide against the template catalog (all modes).** Walk each H2 in the intermediate file and classify it against the shared catalog [`slide-templates.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-templates.md) → *Classification procedure* + each template's *Match* — pick the single best-fit template (or `fallback`), honoring the universal invariant (labeled enumerations → cards, never plain bullets). Every mode classifies; how the pick is recorded and checked differs:
   ...
   - **html-strict** — classification runs inside `slide_model._classify` (via `build_html.py`), which selects the template; nothing to emit here.

3. **Render** by invoking [`skill://antropic-skills:/pptx`](skill://antropic-skills:/pptx). The invocation follows the **base-template 7-stage workflow** described in [`pptx-prompt.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/pptx-prompt.md) §19.3 — the native skill is the executor, this skill is the recipe-bearer. Pass:
   ...
   - the **base template** at [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/base-template.pptx`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/base-template.pptx) (or explicit override) — opened as a **working copy**, not as a theme reference,
   - the **icon library** rooted at the base template's `ppt/media/icon-*.svg` (15 branded line-art SVGs, see [`pptx-prompt.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/pptx-prompt.md) §17),
   - the **visual spec** [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/pptx-prompt.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-strict/pptx-prompt.md) (the layout recipes the skill emits against).
...
4. Verify `talks/<Talk>/output/final.<style>.pptx` exists and is non-empty, then **copy it to the canonical `talks/<Talk>/output/final.pptx`** (the deliverable the reverse pipeline reads). The suffixed per-style deck persists for comparison.
   ...
   **When `style == strict`, snapshot the as-generated geometry baseline** for the learning loop — the deck before any human touches it:
   ...
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/pptx-learn/learn_patterns.py inventory \
     talks/<Talk>/output/final.pptx-strict.pptx \
     -o talks/<Talk>/output/final.generated.geometry.json
   ```

   This per-shape geometry snapshot survives in-place editing of the deck and is the baseline [`talksmith:pptx-learn`](${CLAUDE_PLUGIN_ROOT}/skills/pptx-learn/SKILL.md) diffs the human-edited deck against. Skip for pptx-free-form/html-strict (nothing to learn against).
...
6. **Audit `<p:pic>` aspect ratios.** Run [`audits/aspect_ratios.py`](audits/aspect_ratios.py) against the rendered deck:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/aspect_ratios.py \
   ...
   **Then run [`audits/palette_fonts.py`](audits/palette_fonts.py) — only when `style == strict`.** This enforces the strict template's §2 palette + §3.1 font set (a LAYOUT-CONFORMANCE concern — see [`slide-design.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md)). **Skip for `pptx-free-form` and `html-strict`** — their slides 2+ deliberately have no fixed palette/font, so this audit does not apply (for free-form the fixed cover is still checked by `audits/cover_fidelity.py`; html-strict produces no deck, so no deck-parsing audit runs there at all). For strict:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/palette_fonts.py \
     talks/<Talk>/output/final.pptx
   ```
   ...
   Then run [`audits/cover_fidelity.py`](audits/cover_fidelity.py) to confirm slide 1 of `final.pptx` is byte-equivalent to slide 1 of the style's `<base_template_path>` (modulo only the four cover substitution slots — title, subtitle, author+date, logo image basename; defined in strict §4.3 and free-form §2):

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/cover_fidelity.py \
     talks/<Talk>/output/final.pptx \
     <base_template_path>
   ```
   ...
   **Then run [`audits/layout_fit.py`](audits/layout_fit.py) — only when `style == strict`.** When `style == free-form`, skip this audit entirely (free-form has no §15.5 emit-rules table to predict against). For strict:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/layout_fit.py \
     talks/<Talk>/final.md \
     talks/<Talk>/output/final.pptx
   ```
   ...
   **Then run [`audits/icon_coverage.py`](audits/icon_coverage.py) — only when `style == strict`.** For strict:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/icon_coverage.py \
     talks/<Talk>/final.md \
     talks/<Talk>/output/final.pptx
   ```
   ...
   Then run [`audits/block_coverage.py`](audits/block_coverage.py) to confirm every load-bearing block from `final.md` survived into the rendered deck:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/block_coverage.py \
     talks/<Talk>/final.md \
     talks/<Talk>/output/final.pptx
   ```
   ...
   Then run [`audits/notes_coverage.py`](audits/notes_coverage.py) (shared floor — **strict and free-form**) to confirm speaker notes were not dropped:

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/audits/notes_coverage.py \
     talks/<Talk>/final.md \
     talks/<Talk>/output/final.pptx
   ```
...
8. Report: `style: <pptx-strict|pptx-free-form|html-strict>`, slide count, image references resolved, `aspect_audit: <ok|N fail>`, `palette_fonts: <ok|N fail|skipped:non-strict>`, `cover_fidelity: <ok|N diff>`, `layout_fit: <ok|N mismatch|skipped:non-strict>`, `block_coverage: <ok|N drop>`, `slide_previews: <count|failed>`, any warnings surfaced by `skill://antropic-skills:/pptx`.

### Render flow — branches by style

The skill is invoked with a mandatory `style:` parameter (the orchestrator asks the presenter every Step 8 entry — never persisted to `final.md`). **The skill owns the entire render loop end-to-end** — including the visual critique iterations for `pptx-strict`. The orchestrator hands off, displays the progress checklist as stage events arrive, and reads the closing report. It does not know whether strict iterates internally or not.

> **Stream the phases — mandatory in every mode (pptx-strict, pptx-free-form, html-strict).** The render must surface its **processing phases as they happen** — the presenter sees the phase progress advance, never a multi-minute *"Multitasking…"* with no output. Two hard rules:
...
#### `style: pptx-strict` — multi-cycle critique loop *(skill-internal)*
...
| **CONTROL** | `[cycle N/3] CONTROL` | Build-time audits: aspect ratios, block coverage, notes coverage, palette/fonts, cover fidelity, layout fit, OOXML invariants (§19.4 of `pptx-strict/pptx-prompt.md`). | All audits exit 0 → FEEDBACK. Any non-zero → straight to REGENERATE for this cycle (no visual review on a broken render). |
| **FEEDBACK** | `[cycle N/3] FEEDBACK` | Skill dispatches a multimodal sub-agent (via `Agent` tool) to walk the shared critique rubric [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md) on every slide PNG (multimodal `Read` of rasterized pixels — XML inspection forbidden). **Strict selects all five categories — CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION + LAYOUT-CONFORMANCE** — applying the strict elaborations in `pptx-strict/pptx-prompt.md` §20. Each finding: `slide N · <catalog-id> · <description> → fix this iteration \| defer because <reason>`. **Strict runs autonomously** — no presenter prompts at any point. Editorial-judgement findings get `defer` and surface in the closing report. | List empty OR all entries `defer` → cycle done, exit loop. Any `fix this iteration` → REGENERATE. |
...
#### `style: pptx-free-form` — single pass *(skill-internal)*
...
|---|---|---|
| **GENERATE** | Full pipeline against [`pptx-styles/pptx-free-form/`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/pptx-free-form/) → `final.pptx` + `.layout-log.md` + slide previews. | Output written. |
...
The CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION practices in [`slide-design.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-design.md) are a self-review checklist for the presenter's human pass — the skill does **not** walk them for free-form. (Automated critique lives in pptx-strict; html-strict runs its own.)

#### `html-strict` on `draft.md` — the in-progress live view *(auto-fired, code-rendered)*
...
Degrades gracefully: a render error → the script says so and the orchestrator reports the live view is unavailable (never fatal — it's a convenience).

### Presenter-facing progress checklist

**A live checklist is mandatory in every render mode (pptx-strict, pptx-free-form, html-strict).** Rendering runs 30 s – 3 min; silence reads as a hang. The orchestrator **must** show a live, todo-list-style checklist on render entry and **update it in place** — flipping each item to `[⟳]` when its phase starts and `[✓]` the moment it completes — so the presenter always sees what is happening now and what is done. Never run a render with no visible progress. State markers: `[ ]` pending, `[⟳]` in progress, `[✓]` done, `[—]` skipped / not-applicable, `[✗]` failed.
...
**`style: pptx-strict` checklist** (5 items; *Reviewing slides* and *Applying fixes* update their cycle counter on each rotation):
...
**`style: pptx-free-form` checklist** (3 items; single pass):
...
Item ↔ phase mapping (html-strict in-progress):
...
**Iteration passes.** Cycle 2 and 3 (pptx-strict) / cycle 2 (html-strict) prefix every emission with `[cycle N/M] <PHASE>` and emit **per phase and per batch within FEEDBACK**, so a multi-cycle render shows continuous forward motion (build → review 10/29 → review 20/29 → 4 fixes → re-build → …) rather than one opaque spinner. Free-form is single-pass (no cycles). Same suppression rule — these are stage events for the checklist, not chat content.

**No-default policy on missing `style:`.** The `style:` invocation parameter is mandatory. If it is absent or empty on skill entry, the skill emits `[pptx 0/8] FAILED: style: invocation parameter missing` and stops — it does **not** fall back to `pptx-strict` or any other value. The orchestrator's job is to ask the presenter (Step 8 step 1) and pass an explicit answer; if the skill is reached without one, the orchestrator must re-ask. There is no defense-in-depth default — silent fallback was the bug, the loud failure is the fix.
...
- **Never read or modify `draft.md`** — except read-only in the `html-strict --draft` in-progress view, which reads `draft.md` (never writes it) to render the live view. In every other invocation `draft.md` is untouched. No mode ever *modifies* `draft.md`.
