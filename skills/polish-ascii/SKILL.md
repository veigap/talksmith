...
- **ASCII block** — a fenced block whose opening fence is exactly ` ```ascii `. That tag is the whole rule: no payload inspection, no glyph sniffing, no other info string. A fence tagged with a real language (`python`, `json`, `bash`, …), tagged `text`, or **left untagged** is code, data or prose, and is never detected.

  **Why there is no fallback.** Guessing from the payload was tried and removed. The heuristic accepted any untagged fence containing `->`, `|` or `+--`, or simply three or more lines — and on a deck that quotes prompts and code it found 16 blocks of which 2 were diagrams. Step 6 *rasterizes what it detects and deletes the source fence*, so the other 14 would have become unreadable pictures. Tagging a fence costs five characters; a wrong guess costs the content. If a diagram is not being rendered, the fix is always the same: tag its fence ` ```ascii `.
