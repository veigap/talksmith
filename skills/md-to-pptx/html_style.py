...
(function(){
  ...
  function show(n){ if(!figs.length)return; i=Math.max(0,Math.min(figs.length-1,n));
    figs.forEach(function(f,k){ f.classList.toggle('cur',k===i); }); var c=document.getElementById('pcount'); if(c)c.textContent=(i+1)+' / '+figs.length; }
  function present(on){ if(on){ root.setAttribute('data-present',''); show(i); } else { root.removeAttribute('data-present'); } }
