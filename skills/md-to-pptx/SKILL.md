...
0. **Preview is a style, not an exception.** `preview` is one of the render styles
   ([`preview/pptx-prompt.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/preview/pptx-prompt.md)); it differs only in **substrate** — a code wireframe, not a native `.pptx`. When the resolved style is **`preview`** (the orchestrator's Step-5.5 passes `style: preview`; the legacy `preview: true` invocation is accepted as an alias for it), the render is [`build_preview.py --talk talks/<Talk>`](${CLAUDE_PLUGIN_ROOT}/skills/md-to-pptx/build_preview.py) (no native `pptx` skill, no base-template, no CONTROL audits) followed by its **own light critique loop** (CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION, ≤2 cycles, per-slide, findings surface) — see *preview — Step-5.5 draft preview* in *Render flow*. All other steps below (the native-`pptx` pipeline) apply to the `strict` / `free-form` styles.
