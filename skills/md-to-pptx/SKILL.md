...
0. **Preview branch.** If the invocation carries **`preview: true`**, skip the style ask and the strict/free-form pipeline below — the preview renders via [`build_preview.py --talk talks/<Talk>`](${CLAUDE_PLUGIN_ROOT}/skills/md-to-pptx/build_preview.py) (no native `pptx` skill), then runs **its own light critique loop** (CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION, ≤2 cycles, per-slide, findings surface) — see *`preview: true` — Step-5.5 draft preview* in *Render flow*. `preview:` overrides `style:` if both are present. Otherwise continue with normal style resolution:
...
2.5. **Classify each slide against the template catalog (all modes).** Walk each H2 in the intermediate file and classify it against the shared catalog [`slide-templates.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-templates.md) → *Classification procedure* + each template's *Match* — pick the single best-fit template (or `fallback`), honoring the universal invariant (labeled enumerations → cards, never plain bullets). Every mode classifies; how the pick is recorded and checked differs:
   ...
   - **preview** — classification runs inside `build_preview.py` (`_classify`), which selects the render function; nothing to emit here.
...
6. **Audit `<p:pic>` aspect ratios.** Run [`audit_aspect_ratios.py`](audit_aspect_ratios.py) against the rendered deck:
   ...
   **Then run [`audit_palette_fonts.py`](audit_palette_fonts.py) — only when `style == strict`.** This enforces the strict template's §2 palette + §3.1 font set (a LAYOUT-CONFORMANCE concern — see [`slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md)). **Skip for `free-form` and `preview`** — their slides 2+ deliberately have no fixed palette/font, so this audit does not apply (for free-form the fixed cover is still checked by `audit_cover_fidelity.py`; preview produces no deck, so no deck-parsing audit runs there at all). For strict:
...
7. **Render per-slide critique PNGs.** Rasterize every slide to `talks/<Talk>/output/.critique/slide-NN.png` (zero-padded 2-digit slide number, `01` … `NN`). These PNGs are critique-only — not referenced from `final.pptx`, not part of the deliverable — and exist so the FEEDBACK sub-agent can walk the shared critique rubric on actual pixels rather than slide-XML inspection (see [`slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md) and *Render flow* above).
...
The CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION practices in [`slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md) are a self-review checklist for the presenter's human pass — the skill does **not** walk them for free-form. (Automated critique lives in strict; the throwaway preview runs its own.)
...
| **FEEDBACK** | `[cycle N/2] FEEDBACK` | Multimodal walk of CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION on the changed slide PNGs, per the shared rubric. Findings surface (see REGENERATE). |
...
| Reviewing slides (cycle N) | FEEDBACK finishes walking CONTENT + TEMPLATE + AESTHETIC + DISTRIBUTION on the changed slide images. |
