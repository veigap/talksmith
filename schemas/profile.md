...
| `Presenter` | **Required** | Identity record — name, role, organization (e.g. "Paulo Veiga, Lecturer, Universidad Austral"). **For shared-repo setups** (multiple presenters teaching the same subject — see `README.md` → *One shared repo per subject*), list every team presenter on its own line. The Editor copies the field into every Talk's `draft.md` frontmatter as the default `presenter`; whichever presenter is giving a given class overrides per-Talk in `draft.md` frontmatter at Step 4 or Step 5 Review. |
