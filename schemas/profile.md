...
Captures per-presenter defaults that apply across every Talk in this working directory: the **subject** the working directory is dedicated to (one working directory per subject — see [`README.md`](${CLAUDE_PLUGIN_ROOT}/README.md) → *One repo per subject*), who is delivering, how presentations are typically consumed, who the typical audience is, default total duration, presentation language, and whether the subject has an institution logo for rendered deck covers. Read once at session start and kept in context across all role work. Step 4 (Draft) reads these silently to populate `draft.md` frontmatter — it does **not** re-prompt for any field listed here.
...
| `Presenter` | **Required** | Identity record — name, role, organization (e.g. "Jane Doe, Lecturer, Example University"). **For shared-repo setups** (multiple presenters teaching the same subject — see `README.md` → *One repo per subject*), list every team presenter on its own line. The Editor copies the field into every Talk's `draft.md` frontmatter as the default `presenter`; whichever presenter is giving a given class overrides per-Talk in `draft.md` frontmatter at Step 4 or Step 5 Review. |
...
| Exists but missing one or more canonical section headings (e.g. a hand-edited file that dropped `## Audience defaults`) | Treat the file as needing rebuild: re-bootstrap from the *Canonical empty form* below, **preserving any content under the canonical headings that did exist** (copy it into the rebuilt file under the same heading). Never silently drop presenter content. Then proceed as the "exists but every section is empty" case for whatever required sections came up empty after the rebuild. |
