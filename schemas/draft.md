...
| Optional slide hints `<!-- template: <type> -->` / `<!-- layout: <value> -->` / `<!-- format: <value> -->` / `<!-- reveal: together -->` / `<!-- aside: [left\|right] ![alt](path) -->` (author-written under a slide's `##` heading; see [`editor.md`](${CLAUDE_PLUGIN_ROOT}/agents/editor.md) → *Step 4*) | Present when the Editor had a specific intent; optional | **Preserved verbatim** (Step 6 copies them through) — read by the `md-to-deck` FILL as directives when mapping to `slide-model.json`. The `aside:` hint embeds a real `![alt](path)` ref, so the image-ref rewrite in the row above applies to it too |
