...
| `<!-- generate-image: <side> \| <description> -->` directives (sparse-text atmospheric aside; author-written under a slide's `## Content`; see [`editor.md`](${CLAUDE_PLUGIN_ROOT}/agents/editor.md) → *Suggest an atmospheric aside*) | Present when the Editor proposed a generated aside; the `<description>` is a **short high-level** line | Replaced by `<!-- aside: <side> ![alt](images/<slide-id>-<n>-aside.png) -->` + a `<!-- generate-source: <description> -->` echo, **once the image was generated** (Step 6 step 1b, [`image-illustrator`](${CLAUDE_PLUGIN_ROOT}/agents/image-illustrator.md)). Left verbatim if no image capability was present or the directive was mis-authored |
| `![alt](path)` image refs | Present, may point at corpus-companion paths, external paths, etc. A **video** is written the same way — a local `.webm`/`.mp4`/`.mov` clip, or a YouTube link — and plays (autostarting on slide entry) in the HTML deck; see [`slide-model.md`](${CLAUDE_PLUGIN_ROOT}/schemas/slide-model.md) → *`design` + `media`* | Present, **all** rewritten to `images/<basename>` (remote URLs — a YouTube link included — are the only exception, left untouched) |
| Optional slide hints `<!-- template: <type> -->` / `<!-- design: <value> -->` / `<!-- format: <value> -->` / `<!-- reveal: together -->` / `<!-- aside: [left\|right] ![alt](path) -->` (author-written under a slide's `##` heading; see [`editor.md`](${CLAUDE_PLUGIN_ROOT}/agents/editor.md) → *Step 4*) | Present when the Editor had a specific intent; optional | **Preserved verbatim** (Step 6 copies them through) — read by the `md-to-deck` FILL as directives when mapping to `slide-model.json`. The `aside:` hint embeds a real `![alt](path)` ref, so the image-ref rewrite in the row above applies to it too |
...
| `talksmith:md-to-deck` skill (reader of `final.md`) | Step 7 (Render) | FILL `output/slide-model.json` (LLM decomposition), then render it — mechanically to HTML (`build_html.py`), or via the session's official `pptx` skill for the `.pptx` styles. Read-only on `final.md`. |
...
5. **Step 7 (Render, optional).** The md-to-deck skill FILLs `output/slide-model.json` from `final.md` (LLM), then renders it: `build_html.py` → `output/html/index.html`. If the render was asked for them, `export_pdf.py` and `export_pptx.py` then measure that deck into `output/final.pdf` and `output/final.pptx`. Neither `draft.md` nor `final.md` is modified.
