...
| [`talksmith_bugs.md`](talksmith_bugs.md) | On any defect, in any step — before appending, to deduplicate | Orchestrator (**sole writer**, append-only). See *Defect log* below. |
...
## Defect log — `talksmith_bugs.md`

**Mandatory, every step, no exceptions.** Every inconsistency or malfunction *in Talksmith itself* gets documented in `talksmith_bugs.md` at the **root of the working directory** (sibling of `CLAUDE.md`, `config/`, `talks/`) — so it can be reported upstream and fixed. Full spec, entry fields and canonical empty form: [`${CLAUDE_PLUGIN_ROOT}/schemas/talksmith-bugs.md`](${CLAUDE_PLUGIN_ROOT}/schemas/talksmith-bugs.md). Create the file from that schema's empty form the first time you need it; never scaffold it empty.
...
- **Never narrate it in chat** (per *Speak human, not internal*). At most one plain line when a step closes with defects logged: *"I hit a couple of rough edges in the tool; they're written down in `talksmith_bugs.md`."* Nothing when the log stayed empty.
