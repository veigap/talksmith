...
The `@`-import is a CLI convention and **some environments — notably Cowork — do not expand it**, so the spec may be missing even though this stub loaded. **Verify:** you should see the spec's heading *"Talksmith — Presenter Agent (orchestrator spec)"* and its Steps 0–8. If you do **not**, **Read it now** before continuing: `Read ${CLAUDE_PLUGIN_ROOT}/orchestrator.md`. If `${CLAUDE_PLUGIN_ROOT}` is unset — it is empty in some environments, the VS Code extension among them — read the spec straight from the install: `~/.claude/plugins/marketplaces/talksmith/orchestrator.md`, or `~/.claude/plugins/cache/talksmith/talksmith/<version>/orchestrator.md`. Those are the canonical locations; only search for `talksmith/orchestrator.md` under the Claude Code plugins directory if neither exists. **Note that root once and reuse it** — every script the workflow calls by path (`build_html.py`, `model_freshness.py`, `audits/*.py`) hangs off it, and re-deriving it with `find` at each step is the tax this line exists to remove. If it is genuinely unfindable, stop and tell the user to re-install the plugin (`/plugin install talksmith@talksmith`) and reload. Do not proceed without the spec loaded.
...
| `talksmith-bugs.md` | Inconsistencies and malfunctions Talksmith hit while running — each with context, a repro, and a **suggested** fix, so they can be reported upstream and fixed. Not feedback about your talk. | Orchestrator · any step, on demand |
