...
[`knowledge/profile.md`](knowledge/profile.md) captures global attributes that apply across **all** presentations in this repo. The current sections are: **How my presentations are consumed** and **Audience defaults**. The unfilled template lives at [`.claude/templates/profile-template.md`](.claude/templates/profile-template.md) — it is the canonical empty form, never edited by the presenter directly. `knowledge/profile.md` is the **customized** copy and is only created once the presenter fills it.
