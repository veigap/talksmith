...
| [`skills/`](skills/) | Five skills — `ingest`, `ascii-to-svg`, `polish-ascii`, `find-open-notes`, `md-to-pptx`. Skill names are namespaced as `talksmith:<skill>` in their SKILL.md frontmatter. |
| [`scripts/`](scripts/) | Deterministic helper scripts the subagents shell out to (currently [`feedback_cycle.py`](scripts/feedback_cycle.py) — Step 5/6 bookkeeping). Not user-facing skills; invoked directly by `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/...` from agent specs. |
