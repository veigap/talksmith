...
Talksmith renders to a **styled static HTML site** as well as `.pptx`: `skills/md-to-deck/build_html.py` parses + classifies via `slide_model.py` and renders the catalog templates via `html_style.py` (content-matched Material Symbols icons via `icon_fetch.py`), wrapping them in a **[Reveal.js](https://revealjs.com/)** shell vendored + inlined under `skills/md-to-deck/vendor/reveal/`. Reveal owns navigation, deck-to-window scaling, transitions, the slide overview, **speaker notes** (source `### Speaker notes` → `<aside class="notes">`, shown with `s`), and **PDF export** (`?print-pdf`); the only custom presentation code is a per-slide content-fit (scale-to-fill-width + fit-height, which Reveal/CSS can't do). Unlike the native `.pptx` render, this is deterministic — icons, callout boxes, code surfaces, and card strips always render — and it drives both the live **in-progress view** (from `draft.md`, auto-synced during drafting) and the **HTML deliverable** (from `final.md`). An optional `<!-- template: X -->` comment under a slide heading forces that template.
...
> ```
> python3 skills/md-to-deck/build_html.py --talk tests/skills/md-to-deck -o tests/skills/md-to-deck/style-reference.html
> ```
