...
| [`skills/`](skills/) | Five skills — `ingest`, `ascii-to-svg`, `polish-ascii`, `feedback-cycle`, `md-to-pptx`. Skill names are namespaced as `talksmith:<skill>` in their SKILL.md frontmatter. The `feedback-cycle` skill carries both its `feedback_cycle.py` CLI (stamp / close / mirror-row / find-closed-unmirrored / rescue-open) and the companion `find_open_notes.py` detector — Step-5/Step-6 mechanical bookkeeping consolidated into one skill. |
