...
**Code panels** are the one place a second vendored library runs: **highlight.js** (`vendor/highlight/`, BSD-3, 37 grammars) tokenizes `pre.cbcode > code` in the page, and is inlined **only into decks that contain a code panel**. Python does not tokenize — it maps the model's `language` to a grammar id and dedents the snippet (`_codeprep`), and the look of every token is `theme.css`. **No snippet is ever truncated:** the panel is bounded in CSS and `fitCode` (beside `fitContent` in the deck script) shrinks its type until every line fits; past ~28 lines the render warns that the slide wants splitting. The `.pptx` path is untouched by all of this: it has no browser to run a tokenizer in and keeps the light mono surface its style spec describes.
