...
The two styles are not better/worse — they trade *predictability* for *expressive range*. The same fork can carry Talks in both styles.
...
Default when absent: `strict` (the safer, more constrained mode). The Editor asks the presenter at Step 1 (Frame) per [CLAUDE.md](../../CLAUDE.md) → *Step 1*, proposing 2 candidates derived from the Step-1 briefing's signals (deck-purpose, audience, delivery context).
...
2. **Color palette (§2 in each spec)** — every `<a:srgbClr val="…"/>` in the rendered deck is in the §2 palette. Audited at CONTROL by [`audit_palette_fonts.py`](../../.claude/skills/md-to-pptx/audit_palette_fonts.py).
...
The **render cycle itself** (GENERATE → CONTROL → FEEDBACK → REGENERATE, 3-cycle cap) is style-agnostic and lives in [`CLAUDE.md`](../../CLAUDE.md) → *Render cycle*. What differs per style is the **content** of CONTROL (which audits fire) and the **content** of FEEDBACK (which rubric the orchestrator walks):
...
| FEEDBACK | 12-practice rubric ([`CLAUDE.md`](../../CLAUDE.md) → *Post-render visual review*) | 8-practice free-form design rubric (`free-form/pptx-prompt.md` §6) |
...
2. Author `config/pptx-styles/<name>/pptx-prompt.md` — self-contained spec including the four floor sections (canvas, palette, fonts, cover recipe). Copy from `strict/` or `free-form/` and modify.
3. Ship the starting deck `config/pptx-styles/<name>/base-template.pptx` honoring the floor (cover slide + palette + fonts + white bg).
4. Add a row to the *Available styles* table above.
5. Extend the `style:` enum in [`.claude/schemas/draft.md`](../../.claude/schemas/draft.md) to include `<name>`.
6. Update [`.claude/skills/md-to-pptx/SKILL.md`](../../.claude/skills/md-to-pptx/SKILL.md) to branch on the new value (load the right spec, point at the right base-template, run the right rubric).

No other style's files need to change. That's the point of the split.

## Cross-refs from style-agnostic prose

Documents outside `config/pptx-styles/` (CLAUDE.md, SKILL.md, the audit scripts, the schemas) reference style-specific spec sections by writing `config/pptx-styles/<style>/pptx-prompt.md §X` where `<style>` is resolved per the active Talk's `style:` field. The style-agnostic audit scripts (aspect-ratio, block-coverage, palette-fonts) take the active style's `pptx-prompt.md` path as an input where needed; the layout-fit audit is `strict`-only and lives unchanged in the skill.
