...
| **preview** | [`preview/`](preview/) | [`preview/pptx-prompt.md`](preview/pptx-prompt.md) | *none* — code substrate (Pillow), no base-template, no `.pptx` | The Step-5.5 fast, throwaway look at a pre-Polish `draft.md` — flip through slide order + rough content before committing to a full render. **Cowork-independent.** |

The styles are not better/worse — strict and free-form trade *predictability* for *expressive range* and both produce a native `.pptx`; **preview** is the odd one out by *substrate* (it renders a code wireframe to PNGs, no deck) but is otherwise a normal style: selected the same way, template-aware, with its own critique loop. The same subject repo can carry Talks rendered in any of them.
