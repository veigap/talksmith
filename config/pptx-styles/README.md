...
| **html** | [`html/`](html/) | [`html/pptx-prompt.md`](html/pptx-prompt.md) | *none* — code substrate (`build_html.py`), no base-template | A shareable, presentable **Reveal.js deck** from `final.md` — full strict styling, a cover slide, present mode (arrow keys / full screen), speaker notes (`s`), and PDF export (`?print-pdf`). Deterministic; the styled layer always renders. **Cowork-independent.** |

The styles trade *predictability* for *expressive range* and *substrate*: **strict** and **free-form** produce a native `.pptx` (Cowork); **preview** and **html** are the same code-generated **styled-HTML** renderer ([`build_html.py`](${CLAUDE_PLUGIN_ROOT}/skills/md-to-deck/build_html.py)) — `preview` for a fast throwaway look at `draft.md`, `html` for a presentable deliverable from `final.md`. All are selected the same way, template-aware, and honour the shared catalog + design bar. The same subject repo can carry Talks rendered in any of them.
