...
`Card heading`; an **unordered** labeled set → the §7.2 card grid. The ordinal/label becomes
the card number/heading; the description becomes the body. Never emit the label as an inline
paragraph prefix (`**Paso 1.** …`) — the named hierarchy must stay visually scannable.
