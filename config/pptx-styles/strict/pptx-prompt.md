...
Only after all four pass: render the deck to PNG via `soffice --headless --convert-to pdf` + `pdftoppm` (or the native skill's slide-to-image endpoint), then walk the shared quality catalog [`../slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md) — strict selects all four categories (CONTENT + AESTHETIC + DISTRIBUTION + LAYOUT-CONFORMANCE) per the [`render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md) matrix, with the strict elaborations in §20 — as the cycle's FEEDBACK phase.
...
**The practices strict walks live in the shared catalog** [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md). Strict selects **all four categories — CONTENT + AESTHETIC + DISTRIBUTION + LAYOUT-CONFORMANCE**, at cycle cap 3, per the per-format matrix in [`render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md). §20.2 below is no longer a standalone rubric; it records only the strict-specific *elaborations* keyed by catalog id (the depth the catalog deliberately omits). §20.1 (the block-coverage gate), and the shared walk discipline / aesthetic note / declare-clean contract now live in `slide-quality.md`; strict inherits them and keeps only the notes below.
...
The practice list itself lives in [`../slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md). Strict walks all four categories; below is the extra depth strict applies to specific catalog entries — the catalog keeps the one-line intent, these notes keep the strict rigor.
...
Walk discipline, the aesthetic note, and the declare-clean / closing-report contract are shared and now live in [`../slide-quality.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/slide-quality.md) → *Walk discipline* / *When to declare clean*. Strict inherits them verbatim.
