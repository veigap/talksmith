...
Only after all four pass: render the deck to PNG via `soffice --headless --convert-to pdf` + `pdftoppm` (or the native skill's slide-to-image endpoint), then walk the shared critique rubric [`../render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md) — strict selects all four categories (CONTENT + AESTHETIC + DISTRIBUTION + LAYOUT-CONFORMANCE), with the strict elaborations in §20 — as the cycle's FEEDBACK phase.
...
**The practices strict walks live in the shared catalog** [`${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md). Strict selects **all four categories — CONTENT + AESTHETIC + DISTRIBUTION + LAYOUT-CONFORMANCE** — per that file's selection matrix (cycle cap 3). §20.2 below is no longer a standalone rubric; it records only the strict-specific *elaborations* keyed by catalog id (the depth the catalog deliberately omits). §20.1 (the block-coverage gate), §20.3 (walk discipline), §20.4 (aesthetic note) and §20.5 (declare-clean) are now stated once in the catalog; strict inherits them and keeps only the notes below.
...
The practice list itself lives in [`../render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md). Strict walks all four categories; below is the extra depth strict applies to specific catalog entries — the catalog keeps the one-line intent, these notes keep the strict rigor.
...
Walk discipline, the aesthetic note, and the declare-clean / closing-report contract are shared and now live in [`../render-modes.md`](${CLAUDE_PLUGIN_ROOT}/config/pptx-styles/render-modes.md) → *Walk discipline* / *When to declare clean*. Strict inherits them verbatim.
