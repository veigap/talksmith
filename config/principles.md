...
- **Wording defects belong at Step 5 (Review).** Rephrasing, tightening, replacing a term, fixing a typo, adjusting tone — these are wording problems. They are cheap to fix as `Presenter feedback` bullets in `draft.md`; the Editor applies them via the `talksmith:feedback-cycle` skill. Wording fixes at Step 8 (renderer) are forbidden — they leak into `final.md` without an audit trail.
- **Rendering defects belong at Step 6 (Polish) or Step 8 (PPTX).** An ASCII diagram that needs SVG promotion, an image that needs consolidation into `images/`, an emoji that needs an icon swap, a callout color, an overflow — these are rendering problems. They are fixed inside the renderer (illustrator skill, md-to-deck skill) and never leak back into `draft.md`.
