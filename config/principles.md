...
- **Wording defects belong at Step 5 (Review).** Rephrasing, tightening, replacing a term, fixing a typo, adjusting tone — these are wording problems. They are cheap to fix as `Presenter feedback` bullets in `draft.md`; the Editor applies them via the `scripts/feedback_cycle.py` helper. Wording fixes at Step 8 (renderer) are forbidden — they leak into `final.md` without an audit trail.
