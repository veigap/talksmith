...
- **Stamp after rendering.** When the render pass finishes, run `polish-ascii stamp-renders --final <final.md> --plan <plan.annotated.json>` so the next pass can tell what changed. Skipping this doesn't corrupt anything — it just re-renders everything next time.
