...
- **One output record per source.** Filename: `research/corpus/<original-filename>.<original-ext>.md`. For web captures: `research/corpus/<folder-name>.web.md`. Companion folder: `research/corpus/<original-filename>.<original-ext>/images/` (drop the `.md`).
...
- **Mark how well you know it.** Every `Inconsistencies / open questions` item is either `[verified]` (you checked it, and you name the check) or `[open question]` (it reads wrong but you could not confirm it). Downstream, only a `[verified]` item authorizes the Editor to change content — so a claim about the outside world you did not actually check is an `[open question]`, always. Before declaring any **Claude / Anthropic model name** unknown or invented, consult the `claude-api` skill; the roster moves faster than the corpus, and a wrong "this model does not exist" makes the Editor delete a correct fact. Contract in `${CLAUDE_PLUGIN_ROOT}/schemas/corpus-record.md` → *Claim discipline*.
- **A truncated excerpt says so.** `Raw / preserved excerpts` promises restorable text. When the original is itself cut off — a clipped PPTX text box, a page captured mid-paragraph — append `<!-- truncated-in-source: no complete version available -->` after the excerpt. Unmarked means complete, and an unmarked truncation sends the Editor looking for a full version that does not exist. Use `<!-- pending: failed: <reason> -->` instead when it was *your extraction* that cut it, so the record stays re-processable.
