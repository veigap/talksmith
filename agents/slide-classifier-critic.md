...
4. **Apply the anti-default escape check** whenever `template` is one of the catalog's three sinks — `concept-breakdown` (or `card-row` / `icon-list`), `content-text`, or `content+cards+image`. Its six questions: dates? ordered? numbers as payload? row-aligned or two groups? an image, shared or per-item? and, on `content+cards+image` specifically, **do the items read across** — two sides of a comparison, or rows over shared factors? A `yes` to any is a re-classification. That last one is the catalog's named most-common miss: `value-columns` carries the picture too, so a shared image is never a reason to collapse aligned columns into card bodies.
