...
- `<!-- layout: <value> -->` — pin the **arrangement within** the chosen template, when the type is right but the default placement isn't. Currently read by `content-image`: `text-left` (default — text left, image right), `image-left` (mirrored — the image leads the eye and the text follows), `image-top` (stacked — image over a short caption). Unlike `aside`, none of these crop, so all three are safe for a diagram the audience must read. An unrecognized value falls back to the template's default rather than erroring.
